...
VOID DumpMemoryA (LPCVOID address, SIZE_T size)
{
    ...
    SIZE_T bytesDone                  = 0;
    WCHAR  hexDump[HEXDUMPLINELENGTH] =
    {
        0
    };
    WCHAR  ascDump[18]                =
...
BOOL StrToBool (LPCWSTR s)
{
    ...
    if ((_wcsicmp (s, L"true") == 0) || (_wcsicmp (s, L"yes") == 0) || (_wcsicmp (s, L"on") == 0) || (wcstol (s, &end, 10) == 1))
    {
        return TRUE;
    }
    else
...
DWORD _GetProcessIdOfThread (HANDLE thread)
{
    ...
    typedef NTSTATUS (WINAPI *PNtQueryInformationThread) (HANDLE thread, THREADINFOCLASS infoclass, PVOID buffer, ULONG buffersize, PULONG used);
...
void GetFormattedMessage (DWORD last_error)
{
    ...
    WCHAR lpMsgBuf[MAX_PATH] =
    {
        0
    };

    FormatMessage (FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, last_error, MAKELANGID (LANG_NEUTRAL, SUBLANG_DEFAULT), lpMsgBuf, MAX_PATH, NULL);
