...
bool CallStack::isInternalModule (const PWSTR filename) const
{
    return wcsstr (filename, L"\\crt\\src\\afxmem.cpp") || wcsstr (filename, L"\\crt\\src\\dbgheap.c") || wcsstr (filename, L"\\crt\\src\\malloc.c") || wcsstr (filename, L"\\crt\\src\\dbgmalloc.c") || wcsstr (filename, L"\\crt\\src\\new.cpp") || wcsstr (filename, L"\\crt\\src\\newaop.cpp")
           || wcsstr (filename, L"\\crt\\src\\dbgcalloc.c") || wcsstr (filename, L"\\crt\\src\\realloc.c") || wcsstr (filename, L"\\crt\\src\\dbgrealloc.c") || wcsstr (filename, L"\\crt\\src\\dbgdel.cp") || wcsstr (filename, L"\\crt\\src\\free.c") || wcsstr (filename, L"\\vc\\include\\xmemory0");
}
...
#error "Visual Leak Detector is not supported on this architecture."
    ...
    while (count < maxdepth)
    {
        ...
        if (!StackWalk64 (architecture, g_currentProcess, g_currentThread, &frame, &currentContext, NULL, SymFunctionTableAccess64, SymGetModuleBase64, NULL))
        {
