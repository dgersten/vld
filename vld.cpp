...
{
    "LdrLoadDll", NULL, VisualLeakDetector::_LdrLoadDll, NULL, NULL, NULL
};
...
VisualLeakDetector::VisualLeakDetector ()
{
    ...
    {
        Report (L"ERROR: Visual Leak Detector could not be installed because thread local" L"  storage could not be allocated.");
        return;
    }
...
VOID VisualLeakDetector::attachToLoadedModules (ModuleSet * newmodules)
{
    ...
    for (ModuleSet::Iterator newit = newmodules->begin (); newit != newmodules->end (); ++newit)
    {
        ...
        if ((refresh == true) && (moduleflags & VLD_MODULE_SYMBOLSLOADED))
        {
            ...
            {
                Report (L"WARNING: Visual Leak Detector: Failed to unload the symbols for %s. Function names and line" L" numbers shown in the memory leak report for %s may be inaccurate.\n", modulename, modulename);
            }
        }
        ...
        if ((moduleflags & VLD_MODULE_EXCLUDED) == 0 &&!(moduleflags & VLD_MODULE_SYMBOLSLOADED) || (moduleimageinfo.SymType == SymExport))
        {
...
VOID VisualLeakDetector::remapBlock (HANDLE heap, LPCVOID mem, LPCVOID newmem, SIZE_T size, bool crtalloc, CallStack **& ppcallstack, const context_t & context)
{
    if (newmem != mem)
    {
...
SIZE_T VisualLeakDetector::getLeaksCount (heapinfo_t * heapinfo)
{
    ...
    for (BlockMap::Iterator blockit = blockmap->begin (); blockit != blockmap->end (); ++blockit)
    {
        ...
        if (heapinfo->flags & VLD_HEAP_CRT_DBG)
        {
            ...
            if ((CRT_USE_TYPE (crtheader->use) == CRT_USE_IGNORE) || (CRT_USE_TYPE (crtheader->use) == CRT_USE_FREE) || (CRT_USE_TYPE (crtheader->use) == CRT_USE_INTERNAL))
            {
...
SIZE_T VisualLeakDetector::reportLeaks (heapinfo_t * heapinfo, Set<blockinfo_t *> & aggregatedLeaks)
{
    ...
    for (BlockMap::Iterator blockit = blockmap->begin (); blockit != blockmap->end (); ++blockit)
    {
        ...
        if (firstLeak)
        {    // A confusing way to only display this message once
...
blockinfo_t * VisualLeakDetector::findAllocedBlock (LPCVOID mem, __out HANDLE & heap)
{
    ...
    for (HeapMap::Iterator it = m_heapMap->begin (); it != m_heapMap->end (); ++it)
    {
        ...
        for (BlockMap::Iterator iter = p_block_map.begin (); iter != p_block_map.end (); ++iter)
        {
            if ((*iter).first == mem)
            {
                // Found the block.
...
NTSTATUS VisualLeakDetector::_LdrLoadDll (LPWSTR searchpath, ULONG flags, unicodestring_t * modulename, PHANDLE modulehandle)
{
    // Load the DLL.
...
NTSTATUS VisualLeakDetector::_LdrLoadDllWin8 (DWORD_PTR reserved, PULONG flags, unicodestring_t * modulename, PHANDLE modulehandle)
{
    // Load the DLL.
...
CONST UINT32 OptionsMask = VLD_OPT_AGGREGATE_DUPLICATES | VLD_OPT_MODULE_LIST_INCLUDE | VLD_OPT_SAFE_STACK_WALK | VLD_OPT_SLOW_DEBUGGER_DUMP | VLD_OPT_START_DISABLED | VLD_OPT_TRACE_INTERNAL_FRAMES | VLD_OPT_SKIP_HEAPFREE_LEAKS | VLD_OPT_VALIDATE_HEAPFREE | VLD_OPT_RELEASE_CRT_RUNTIME;
